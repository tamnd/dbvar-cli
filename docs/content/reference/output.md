...
describes what users get. Pick a format with `-o`, or let dbvar choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
dbvar <command> -o table   # aligned columns for reading
dbvar <command> -o jsonl   # one JSON object per line, for piping
dbvar <command> -o json    # a single JSON array
dbvar <command> -o csv     # spreadsheet friendly
dbvar <command> -o tsv     # tab-separated
dbvar <command> -o url     # just the URL column
dbvar <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
dbvar <command> --fields id,title,url
```
...
```bash
dbvar <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
dbvar <command>            # a table, because this is a terminal
dbvar <command> | wc -l    # JSONL, because this is a pipe
```
